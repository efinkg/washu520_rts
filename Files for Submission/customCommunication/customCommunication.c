...
static int timer_limit = 60000;
static int timer_interval = 1;
#define recordLimit 100
long long int record[recordLimit];
...
static void timer_code(void *arg)
{
	...
	if (timer_count % timer_interval == 0) {
	    ...
	    rtapi_print("Hello\n");
	    //record[timer_count]=rtapi_get_time();
	    /*response = rtapi_inb(0);
            if (response) {
	    	rtapi_print(response);
	    	rtapi_print("\n");
	    }
	    else{
	    	rtapi_print("Reponse is null\n");
	    }*/
	}
